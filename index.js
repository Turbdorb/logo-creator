...
function createLogo(answers) {
    ...
    return `<svg width="300" height="200" version="1.1" xmlns="http://www.w3.org/2000/svg"> ${userShape} <text x="150" y="115" font-size="50" text-anchor="middle" fill="${answers.textColor}">${answers.text}</text> </svg>`
};
...
function init() {
    ...
    .then((answers) => {
        if(answers.text.length > 3) {
            ...
            fs.writeFile(`./examples/${answers.text}logo.svg`, createLogo(answers), (err) => err ? console.log(err) : console.log('Generated logo.svg'));
        }
    });
};
